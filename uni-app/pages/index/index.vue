	...
	<view class="page-index" :class="{ bgf: navIndex > 0 }">
		...
		<view class="header">
			<view class="serch-wrapper flex">
				<view class="logo" style="margin-right: 32rpx;"><text style="font-size:32rpx;color:rgba(255,255,255,1);letter-spacing:2rpx;">精选宝贝</text></view>
			...
			<view style="width: 750rpx;display: flex;align-items: center;justify-content: space-around;margin-top: 32rpx;margin-bottom: 32rpx;">
				...
				<view class="lxdzbox" @tap="open('set')"><text class="lxdztxt">联系店主</text></view>
			</view>
			<tabNav class="tabNav" :class="{ fixed: isFixed }" :tabTitle="navTop" @changeTab="changeTab" @emChildTab="emChildTab" @childTab="childTab"></tabNav>
		</view>
		<!-- #endif -->
		<!-- #ifdef MP -->
		...
		<view class="page_content" v-if="navIndex == 0">
			...
			<block v-if="liveList.length > 0">
				<view style="width: 750rpx;background-color: #fff;margin-top: 20rpx;">
					<view style="width: 718rpx;height: 70rpx;display: flex;align-items: center;justify-content: space-between;">
						<view
							style="width: 320rpx;height: 70rpx;display: flex;align-items: center;justify-content: center;background-image: url(../../static/images/livebg.png);background-size: 320rpx 70rpx;"
						>
					...
					<block v-for="(item, index) in liveList" :key="index">
						<navigator
							style="width: 750rpx;padding: 20rpx 52rpx 20rpx 32rpx;display: flex;"
							:url="'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?custom_params='+customParams+'&room_id=' + item.roomid"
						>
			...
			<view class="spike-box" v-if="spikeList.length > 0">
				<view class="hd">
					...
					<navigator class="more" url="/pages/activity/goods_seckill/index">
						更多
			...
			<view class="index-product-wrapper">
				...
				<view class="loadingicon acea-row row-center-wrapper" v-if="goodScroll"><text class="loading iconfont icon-jiazai" :hidden="loading == false"></text></view>
				<view class="mores-txt flex" v-if="!goodScroll"><text>我是有底线的</text></view>
			</view>
		</view>
		<!-- 分类页 -->
		<view class="productList" v-if="navIndex > 0" :style="'margin-top:' + prodeuctTop + 'px'">
			<block v-if="sortProduct.length > 0">
				<view class="list acea-row row-between-wrapper" :class="is_switch == true ? '' : 'on'">
					<view class="item" :class="is_switch == true ? '' : 'on'" hover-class="none" v-for="(item, index) in sortProduct" :key="index" @click="godDetail(item)">
						<view class="pictrue" :class="is_switch == true ? '' : 'on'">
							<image :src="item.image" :class="is_switch == true ? '' : 'on'"></image>
							<span class="pictrue_log_class" :class="is_switch === true ? 'pictrue_log_big' : 'pictrue_log'" v-if="item.activity && item.activity.type === '1'">
								秒杀
							</span>
							<span class="pictrue_log_class" :class="is_switch === true ? 'pictrue_log_big' : 'pictrue_log'" v-if="item.activity && item.activity.type === '2'">
								砍价
							</span>
							<span class="pictrue_log_class" :class="is_switch === true ? 'pictrue_log_big' : 'pictrue_log'" v-if="item.activity && item.activity.type === '3'">
								拼团
							</span>
						</view>
			...
			<block v-if="sortProduct.length == 0">
				<view class="noCommodity">
					<view class="pictrue"><image src="/static/images/noShopper.png"></image></view>
...
// #ifdef MP-WEIXIN
import { getTemlIds, getLiveList } from '@/api/api.js';
import { SUBSCRIBE_MESSAGE, TIPS_KEY,SHOP_KEY } from '@/config/cache';
...
import { getCategoryList, getProductslist, getProductHot, getIndexProductList } from '@/api/store.js';
...
export default {
	...
	onLoad() {
		...
		if (store.state.app.shopKey) {
			shop_key_now = encodeURIComponent(store.state.app.shopKey);
		} else {
			shop_key_now = encodeURIComponent('5pSMIG2RFGPfzcz5KeCUhQ==');
		}
		this.customParams = encodeURIComponent(JSON.stringify({ path: 'pages/index/index', shop_key: shop_key_now }));
		// #endif
	},
	onShow() {
		let self = this;
	...
	methods: {
		...
		getIndexProductList() {
			...
			getIndexProductList(this.params).then(({ data }) => {
				this.loading = false;
