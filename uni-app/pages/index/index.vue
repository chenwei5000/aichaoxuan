<template>
  <view class="page-index" :class="{ bgf: navIndex > 0 }">
    <!-- 1.0 Header 部分 -->
    <Header :shopInfo="shopName" :config="headerConfig" @searchHeigth="headerHeight" @changeTab="changeTab"> </Header>
    ...
    <view class="page_content" v-if="navIndex == 0" :style="'position: relative; top: ' + scrollHeight + 'px'">
      ...
      <!-- banner -->
      <view class="swiper">
        <swiper indicator-dots="true" :autoplay="true" :circular="circular" :interval="interval" :duration="duration" indicator-color="rgba(255,255,255,0.6)" indicator-active-color="#fff">
        ...
        <view style="width: 750rpx; background-color: #fff; margin-top: 20rpx;">
          ...
          <block v-for="(item, index) in liveList" :key="index">
            <navigator
              ...
              :url="'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?custom_params=shopid&room_id=' + item.roomid"
            >
      ...
      <view class="spike-box" v-if="spikeList.length > 0">
        <view class="hd">
          ...
          <navigator class="more" url="/pages/activity/goods_seckill/index">更多 <text class="iconfont icon-jiantou" hover-class="none"></text></navigator>
        </view>
    ...
    <view class="productList" v-if="navIndex > 0" :style="'margin-top:' + prodeuctTop + 'px'">
      <block v-if="sortProduct.length > 0">
        <view class="list acea-row row-between-wrapper" :class="is_switch == true ? '' : 'on'">
          <view class="item" :class="is_switch == true ? '' : 'on'" hover-class="none" v-for="(item, index) in sortProduct" :key="index" @click="godDetail(item)">
            ...
            <view class="text" :class="is_switch == true ? '' : 'on'">
              ...
              <view class="money font-color" :class="is_switch == true ? '' : 'on'"
                >秒 ￥<text class="num">{{ item.price }}</text></view
              >
          ...
          <view class="loadingicon acea-row row-center-wrapper" v-if="sortProduct.length > 0"> <text class="loading iconfont icon-jiazai" :hidden="loading == false"></text>{{ loadTitle }} </view>
        </view>
      </block>
      <block v-if="sortProduct.length == 0">
