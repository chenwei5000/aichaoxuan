<template>
	<view class="tab-box" :style="{
				...
				}" id="tab-box" v-if="tabList.length > 0">
		<view class="horizontal">
			<scroll-view :scroll-x="true" style="white-space: nowrap; display: flex;" scroll-with-animation :scroll-left="slider.scrollLeft">
				...
				<view class="item" :class="[itemNumber > 0?'w'+itemNumber: '']" :style="{'color': index === activeIndex ? activeColor : itemColor,'line-height': height+'rpx','height':(Number(height) - 4)+'rpx'}" :id="'tab_'+index" @click="tabClick(index)">{{ item.text || item }}</view>
				</block>
				<view class="underline" :style="'transform:translateX(' + slider.left + 'px);width:' + slider.width + 'px;background-color: ' + activeColor + ';'"></view>
			</scroll-view>
		</view>
	</view>
</template>
